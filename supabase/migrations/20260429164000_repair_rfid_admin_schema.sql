...
ALTER TABLE public.races DROP CONSTRAINT IF EXISTS races_difficulty_level_range;
ALTER TABLE public.races ADD CONSTRAINT races_difficulty_level_range CHECK (difficulty_level BETWEEN 1 AND 5);

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'rfid_timing_results_race_id_fkey'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'rfid_timing_results_registration_id_fkey'
  ) THEN
    ALTER TABLE public.rfid_timing_results
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'gmcap_import_sources_race_id_fkey'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'race_organizers_race_id_fkey'
  ) THEN
...
DROP POLICY IF EXISTS "Organizers can view GMCAP import source" ON public.gmcap_import_sources;
...
DROP POLICY IF EXISTS "Organizers can create GMCAP import source" ON public.gmcap_import_sources;
...
DROP POLICY IF EXISTS "Organizers can update GMCAP import source" ON public.gmcap_import_sources;
...
DROP POLICY IF EXISTS "Organizers can delete GMCAP import source" ON public.gmcap_import_sources;
